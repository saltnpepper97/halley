...
impl AtomicDrmSurface {
    ...
    pub fn commit<'a>(
        ...
        let primary_flags = if event {
            // Halley multi-output startup/DPMS wake can stall badly when the
            // first modeset-style commit on a secondary output blocks the main
            // compositor thread. Try NONBLOCK first so drivers that support it
            // can wake outputs without serializing startup on the slowest link.
        ...
        let fallback_flags = if event {
            AtomicCommitFlags::PAGE_FLIP_EVENT | AtomicCommitFlags::ALLOW_MODESET
        } else {
            AtomicCommitFlags::ALLOW_MODESET
        };

        let result = match self.fd.atomic_commit(primary_flags, built_req) {
            Ok(()) => {
                debug!(
                    "atomic commit succeeded with flags={:?} crtc={:?}",
                    primary_flags, self.crtc
                );
                Ok(())
            }
            Err(source) if event && primary_flags != fallback_flags => {
                debug!(
                    "atomic commit with flags={:?} failed on crtc={:?}, retrying with flags={:?}: {}",
                    primary_flags,
                    self.crtc,
                    fallback_flags,
                    source
                );
                self.fd.atomic_commit(fallback_flags, req.build()?)
            }
