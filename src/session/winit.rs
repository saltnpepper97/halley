...
pub fn run(explicit_config_path: Option<std::path::PathBuf>) {
    ...
    // A fresh configuration generated by a nested session still owes its card
    // to the first native session, but nested sessions never show it: the card
    // is offered by `tty::run` alone. `halleyctl basics` or Lift reopens it here
        ...
        .insert_source(winit_source, move |event, _, app| match event {
            ...
            WinitEvent::Redraw => {
                ...
                if let (Some(before), Some(after)) = (zoom_scale_before, zoom_scale_after)
                    && before != after
                {
                    if app.clusters.active_on(&output_name).is_none() {
                        if after < before {
                            crate::nodes::reconcile_landmarks_for_zoom(app, &output_name, after);
                        } else {
                            crate::nodes::restore_landmarks_for_zoom(app, &output_name, after);
                        }
                    }
