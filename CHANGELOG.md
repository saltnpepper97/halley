...
- Prevent fullscreen XWayland games from consuming Steam/UI pointer motion
  ...
  unlock hints, and synchronously focusing globally-active clients before
  routing the pointer into them.
...
### Changed
- Render mapped Alt-Tab and Apogee previews directly from client surface trees
  instead of repeatedly capturing full-window textures. Cached textures remain
  as the fallback for collapsed or temporarily unmapped windows.
- Drive every visible live preview from its home output's native presentation
  cadence. Apogee redraws stay local to the output containing the card, while
  mirrored Alt-Tab cards invalidate every output on which they are visible.

### Fixed
- Make ancillary-data socket lengths compile on both glibc and musl by
  converting through the platform field types and keeping control-message
  arithmetic in `usize`.
- Blend fullscreen entry correctly for translucent clients that do not
  advertise opaque regions, rather than holding the outgoing frame for the
  entire transition.
- Preserve windowed and maximized restore endpoints across client fullscreen,
  compositor fullscreen, maximize, and rapid repeated transitions.
- Keep compositor-owned `Mod+F` presentation protocol-windowed so a later
  client fullscreen request produces the state edge applications need to
  reflow nested content, including fullscreen video on secondary outputs.
- Send frame callbacks to every visible Alt-Tab entry, not only the centered
  selection, eliminating the heavy lag on non-focused cards and mixed-refresh
  multi-monitor setups.
- Assign a fresh damage identity whenever a cached preview texture is replaced,
  preventing stale hover rectangles and square corruption in Apogee.
- Route modal keyboard releases so modifiers pressed before Apogee opened are
  released to the client while releases for intercepted keys remain suppressed,
  preventing stuck modifiers and broken typing after closing the overview.
- Clip live and cached Alt-Tab, Apogee, and hover-preview content with the
  overlay card radius in output pixels so texture corners match their borders.

## [v0.6.0] - 2026-08-27

### Added
- Ship the rewritten Smithay compositor with native tty/DRM and nested winit
  backends, per-output frame scheduling, direct scanout, VRR-aware presentation,
  writable output management, gamma control, screencopy, and idle inhibition.
- Restore Halley's spatial field model: independent output cameras, zoom and
  pan, collapsible landmark nodes, Bearings navigation, directional focus,
  the Alt-Tab carousel, and the live multi-monitor Apogee overview.
- Restore clusters as a compositor-owned registry with tiling and stacking
  workspaces, overflow, animated creation and reflow, draggable cores, bloom
  joining, floating members, cross-output movement, and CLI/API controls.
- Add native XWayland lifecycle and presentation policy, including ICCCM/EWMH
  integration, override-redirect popups, pointer constraints, relative motion,
  and animated fullscreen and close transitions.
- Add server-side titlebars, rounded window decoration, configurable shadows
  and backdrop blur, compositor backgrounds, window and layer rules, and
  opening, closing, maximize, fullscreen, and workspace animations.
- Add interactive screenshots, a native ScreenCast/Screenshot portal backend,
  DMA-BUF capture, a source chooser, the public `halley-api` crate, `halleyctl`,
  and the ported Halley Lift launcher.
- Add complete Rune configuration for outputs, input devices and gestures,
  keybinds and repeat policy, cursor behavior, effects, animations, nodes,
  clusters, backgrounds, rules, environment, and autostart.

### Changed
- Replace the pre-0.6 compositor internals with explicit field, cluster,
  presentation, shell, session, backend, and renderer domains backed by the
  new `halley-core`, `halley-config`, and `halley-ipc` crates.
- Replace unversioned pre-0.6 configuration files on first launch after writing
  a backup; later 0.6 configuration additions continue to use versioned
  backfill and migration.
- Keep Gamescope integration outside Halley. Launchers may invoke Gamescope
  directly when desired, but it is not part of the compositor or CLI surface.
- Publish the workspace license and package metadata, vendor the default config
  into `halley-config`, and retain the compositor crate as unpublished while it
  follows a pinned Smithay revision.

### Fixed
- Harden window admission, focus succession, pointer constraints, popup
  placement, X11 stacking, and fullscreen ownership across native Wayland and
  XWayland clients.
- Stabilize renderer damage, cached previews, framebuffer effects, backdrop
  blur, shadows, rounded decorations, scanout fallbacks, page flips, and
  mixed-output presentation.
- Preserve geometry, stack depth, camera ownership, and interaction targets
  through maximize, fullscreen, cluster workspace, collapse, drag, resize, and
  close transitions.
